...
function buildPrompt(scenario, userResponse) {
  const actorBlocks = Object.values(scenario.actors || {}).map((actor) => {
    const addr = actor.leader_address || {};
    const format = addr.format || 'address';
    const text = addr.text || '';
    return `${actor.name} (${format}): ${text}`;
  }).join('\n');

  const instructions = loadInstructions();
  const includeCambodia = Boolean(scenario.actors?.Cambodia);
